...
int main(void) {
    int values[] = {18, -2, 24, 33, 101, 48, 52, 7};
...
CHECK_ITEM:
    ...
    if (values[index] < 0) goto REJECT_ITEM;
...
REPORT:
    ...
    printf("Processed sensor samples: %d accepted, %d rejected.\n",
           accepted, rejected);
    printf("Weighted total: %d.\n", total);
    if (total >= 170) goto TARGET_REACHED;
