...
int main(void) {
    int values[] = {4, 12, 7, 25, 18, 2, 15, 30};
...
CHECK_ITEM:
    ...
    if (values[index] < 3) goto REJECT_ITEM;
    if (values[index] > 26) goto REJECT_ITEM;
...
REPORT:
    ...
    printf("Processed tree heights: %d accepted, %d rejected.\n",
           accepted, rejected);
    printf("Weighted total: %d.\n", total);
    if (total >= 75) goto TARGET_REACHED;
