...
int main(void) {
    int values[] = {18, 35, 67, 24, 42, 11, 95, 30};
...
CHECK_ITEM:
    ...
    if (values[index] < 15) goto REJECT_ITEM;
...
REPORT:
    ...
    printf("Processed delivery times: %d accepted, %d rejected.\n",
           accepted, rejected);
    printf("Weighted total: %d.\n", total);
    if (total >= 190) goto TARGET_REACHED;
